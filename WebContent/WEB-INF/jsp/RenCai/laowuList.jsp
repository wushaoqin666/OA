...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="laowuList">劳务合作列表</a></li>
		<li><a href="toaddLaowu">劳务合作添加</a></li>
	</ul>
	<form id="searchForm" class="breadcrumb form-search" action="#"
		...
		<ul class="ul-form">
			<li><label>客户名称：</label> <input id="idcard" name="idcard"
				class="input-medium" type="text" value="" maxlength="20"></li>
			<li><label>身份证号：</label> <input id="idcard" name="idcard"
				class="input-medium" type="text" value="" maxlength="20"></li>
			<li><label>合作公司：</label> <select id="companyid" name="companyid"
				class="input-medium select2-offscreen" tabindex="-1">
	...
	<table id="contentTable"
		...
		<tbody>
			...
			<tr>
				...
				<td>
				 <c:if test="${u.jobtype==0}">
	                    兼职
	             </c:if>
				<c:if test="${u.jobtype==1}">
	                    全职
	             </c:if>
					<c:if test="${u.jobtype==2}">
	                    外派
	             </c:if>
				</td>
				<td>
				...
				<td>
    				<a href="updateLaoWu?id=${u.id_a }">修改</a>
					<a href="deleteLaoWu?id=${u.id_a }" onclick="return confirmx(&#39;确认要删除该人才信息吗？&#39;, this.href)">删除</a>
				</td>
			</tr>
		</c:forEach>
