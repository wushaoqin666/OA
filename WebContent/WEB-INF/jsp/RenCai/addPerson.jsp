...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="toaddPerson">人才信息添加</a></li>
	</ul>
	<br>
	<form id="inputForm" class="form-horizontal" action="addPerson"
		...
		<table class="table table-bordered table-condensed">
			<tbody>
				<tr>
					...
					<td><select name="person.customer.id"
						class="input-xlarge  select2-offscreen" tabindex="-1">
							<c:forEach items="${customerList }" var="customer">
								<option value="${customer.id }"
									${customer.id ==person.customer.id ?"selected":"" }>${customer.name }</option>
							</c:forEach>
					</select></td>
					<td><label class="control-label">身份证号：</label></td>
					<td><input id="idcard" name="person.idcard"
						class="input-xlarge required" type="text"
						value="${person.idcard }" maxlength="20"> <span
						class="help-inline"><font color="red">*</font> </span></td>
				</tr>

				<tr>
					<td><label class="control-label">求职意向：</label></td>
					<td><input id="jobintension" name="person.jobintension"
						class="input-xlarge required" type="text"
						value="${person.jobintension }" maxlength="256"> <span
						class="help-inline"><font color="red">*</font> </span></td>
					<td><label class="control-label">工作类型：</label></td>
					<td><select name="person.jobtype" class="select2-offscreen">
				...
				<tr>
					...
					<td><input id="forprice" name="person.forprice"
						class="input-xlarge required" type="text"
						value="${person.forprice }"> <span class="help-inline"><font
							color="red">*</font> </span></td>
					<td><label class="control-label">期望工作地：</label></td>
					<td><input id="foraddress" name="person.foraddress"
						class="input-xlarge " type="text" value="${person.foraddress }"
						maxlength="20"></td>
				</tr>
				<tr>
					<td><label class="control-label">工作经历：</label></td>
					<td colspan="3"><textarea id="worked" name="person.worked"
							maxlength="256" class="input-xxlarge " rows="2">${person.worked }</textarea></td>
				</tr>
				<tr>
					<td><label class="control-label">自我介绍：</label></td>
					<td colspan="3"><textarea id="personinfo"
							name="person.personinfo" maxlength="256" class="input-xxlarge "
							rows="3">${person.personinfo }</textarea></td>
				</tr>

				<tr>
					<td><label class="control-label">个人简历：</label></td>
					<td colspan="3"><input id="resumeurl" name="person.resumeurl"
						maxlength="256" class="input-xlarge" type="hidden"
						value="${person.resumeurl }">
						...
						<ol id="resumeurlPreview">
							<li><a
								href="file:///E://hrpms/upload/%E6%99%BA%E9%80%92%E5%93%A5_%E4%B8%AA%E4%BA%BA%E7%AE%80%E5%8E%86.doc"
								url="E://hrpms/upload/智递哥_个人简历.doc" target="_blank">智递哥_个人简历.doc</a>&nbsp;&nbsp;<a
								href="javascript:" onclick="resumeurlDel(this);">×</a></li>
						</ol> <a href="javascript:" onclick="resumeurlFinderOpen();"
						class="btn">添加</a>&nbsp;<a href="javascript:"
						onclick="resumeurlDelAll();" class="btn">清除</a> <script
							type="text/javascript">
							function resumeurlFinderOpen() {//
								var date = new Date(), year = date
										.getFullYear(), month = (date
										.getMonth() + 1) > 9 ? date.getMonth() + 1
										: "0" + (date.getMonth() + 1);
								var url = "/jeesite-master/static/ckfinder/ckfinder.html?type=files&start=files:/company/personInfo/"
										+ year
										+ "/"
										+ month
							...
							function resumeurlSelectAction(fileUrl, data,
									allFiles) {
								var url = "", files = ckfinderAPI
										.getSelectedFiles();
							...
							function resumeurlThumbSelectAction(fileUrl, data,
									allFiles) {
								var url = "", files = ckfinderAPI
										.getSelectedFiles();
								for (var i = 0; i < files.length; i++) {
					...
					<td><select name="person.status" class="select2-offscreen">
				...
				<tr>
					...
					<td colspan="3"><textarea id="remark" name="person.remark"
							maxlength="256" class="input-xxlarge " rows="2">${person.remark }</textarea>
					</td>
				</tr>
			</tbody>
		</table>
